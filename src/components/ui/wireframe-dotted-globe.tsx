...
export default function RotatingEarth({
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: PointerEvent) => {
      // Only treat primary button / first touch as drag start
      ...
      try {
        ...
        // Some older browsers may throw; drag still works via document fallback
      }
    };
    ...
    const handleWheel = (event: WheelEvent) => {
      ...
      const newRadius = Math.max(
        ...
        Math.min(
          radius * 3,
          projection.scale() * (event.deltaY > 0 ? 0.9 : 1.1),
        ),
      );
  ...
  if (error) {
    return (
      <div
        className={`flex items-center justify-center bg-black rounded-2xl p-8 ${className}`}
      >
