...
function imageContainerClass(aspect?: GridOption['imageAspect']): string {
  switch (aspect) {
    case 'square':   return 'relative w-full aspect-square rounded-lg overflow-hidden';
    case 'portrait': return 'relative w-full aspect-[3/4] rounded-lg overflow-hidden';
    case 'landscape': return 'relative w-full aspect-[4/3] rounded-lg overflow-hidden';
    default:         return 'relative w-full aspect-[4/3] rounded-lg overflow-hidden';
  }
}
  ...
  return (
    ...
    >
      ...
      {!option.image && option.icon && (
        <span className="text-3xl shrink-0 w-12 text-center" role="img" aria-label={option.label}>
          {option.icon}
        </span>
      )}
      <div className="flex flex-col flex-1">
        <span className="font-semibold text-stone-800 text-sm leading-snug">{option.label}</span>
        {option.description && (
          <span className="text-stone-400 text-xs leading-snug mt-0.5">{option.description}</span>
        )}
      </div>
    </motion.button>
  );
        ...
        {isSingleColumn ? (
          ...
          >
            {question.options.map((option) => (
              <SingleColumnCard key={option.id} option={option} onAnswer={onAnswer} />
            ))}
          </motion.div>
        ) : isCardColumn ? (
          <motion.div
